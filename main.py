...
def build_html(gift_nifty_str, gift_change_pct, dow_str, headline):
    dot_color = "#16a34a" if gift_change_pct >= 0 else "#dc2626"
    dot = "●"
    return f"""
    <html><body style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;padding:24px;background:#f9fafb;">
      <div style="background:#fff;border-radius:12px;padding:24px;border:1px solid #e5e7eb;">
        <h2 style="margin:0 0 16px;font-size:18px;color:#111;">📊 Morning Market Brief</h2>
        <hr style="border:none;border-top:2px solid #e5e7eb;margin:0 0 16px;">
        <table style="width:100%;border-collapse:collapse;">
          <tr>
            <td style="padding:8px 0;font-size:15px;">
              <span style="color:{dot_color};font-size:18px;">{dot}</span>
              <strong> Gift Nifty</strong> <span style="color:#6b7280;font-size:12px;">(8:45 AM)</span>
            </td>
            <td style="padding:8px 0;font-size:15px;text-align:right;font-weight:bold;">{gift_nifty_str}</td>
          </tr>
          <tr>
            <td style="padding:8px 0;font-size:15px;">
              🇺🇸 <strong>Dow Jones</strong> <span style="color:#6b7280;font-size:12px;">(prev close)</span>
            </td>
            <td style="padding:8px 0;font-size:15px;text-align:right;font-weight:bold;">{dow_str}</td>
          </tr>
        </table>
        <hr style="border:none;border-top:2px solid #e5e7eb;margin:16px 0;">
        <p style="margin:0;font-size:14px;color:#374151;">
          📰 <strong>ET:</strong> {headline}
        </p>
        <p style="margin:16px 0 0;font-size:11px;color:#9ca3af;">
          Auto-generated by market-brief · Mon–Fri 8:55 AM IST
        </p>
      </div>
    </body></html>
    """
